...
#ifndef PATH_CIRCLE_HELPER_H
#define PATH_CIRCLE_HELPER_H
 ...
 */
class Path_Circle_Helper
{
public:
  /**
   * @brief set the path circle from center point
   *
   ...
   * @return raw pointer to the KDL::Path object created in this function. You take ownership
   * and are responsible for its deletion.
   */
  static KDL::Path* circleFromCenter(
  ...
  /**
   * @brief set circle from interim point

   * @throws KDL::Error_MotionPlanning if the given points are colinear.
   * @return raw pointer to the KDL::Path object created in this function. You take ownership
   * and are responsible for its deletion.
   */
  static KDL::Path* circleFromInterim(
      const KDL::Frame& start_pose,
      const KDL::Frame& goal_pose,
...
private:
  Path_Circle_Helper() {}; // no instantiation of this helper class!
...
public:
    ...
    virtual int GetType() const {return 3006;}
};

}
#endif // PATH_CIRCLE_HELPER_H
