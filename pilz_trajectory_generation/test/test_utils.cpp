                              ...
                              const double orientation_tolerance)
{
  ...
  {
    std::cerr << "[ Fail     ] forward kinematics computation failed for link: "
              << link_name << std::endl;
  }
                                        ...
                                        const double rot_angle_tolerance)
{
  ...
  for(trajectory_msgs::JointTrajectoryPoint way_point : trajectory.points)
  {
    ...
    if(!computeLinkFK(robot_model,
                      link_name,
                      way_point_joint_state,
                      way_point_pose))
    {
      std::cerr << "Failed to compute forward kinematics for link in goal constraints \n";
      return false;
    ...
    if(fabs( (goal_start_translation.cross(way_point_pose.translation()) -
              goal_start_translation.cross(start_pose.translation())).norm() )
       > fabs(translation_norm_tolerance))
    {
                           ...
                           const double rot_angle_tolerance)
{
         ...
         (fabs(start_wp_aa.angle()) < fabs(rot_angle_tolerance)) ))
  {
    std::cout << "Rotational linearity is violated. \n" << std::endl
              << "start goal angle: " << start_goal_aa.angle() << "; axis: " << std::endl << start_goal_aa.axis() << std::endl
              << "start waypoint angle: " << start_wp_aa.angle() << "; axis: " << std::endl << start_wp_aa.axis() << std::endl;
...
bool testutils::checkJointTrajectory(const trajectory_msgs::JointTrajectory &trajectory, const pilz::JointLimitsContainer &joint_limits)
{
                                 ...
                                 planning_interface::MotionPlanRequest &req)
{
  ...
  // goal state
  req.goal_constraints.push_back(kinematic_constraints::constructGoalConstraints(goal_state,
                                                                                 goal_state.getRobotModel()->getJointModelGroup(planning_group)));
}
                                                     ...
                                                     const double time_tolerance)
{
  for(std::size_t i = 0; i < res.first_trajectory->getWayPointCount(); ++i)
  {
    ...
    if(res.first_trajectory->getWayPointDurationFromStart(i) !=
       req.first_trajectory->getWayPointDurationFromStart(i))
    {
  ...
  {
    for(const std::string& joint_name : res.second_trajectory->getWayPoint(size_second - i -1).getJointModelGroup(req.group_name)->getActiveJointModelNames())
    {
      // check joint position
...
bool testutils::checkBlendingJointSpaceContinuity(const pilz::TrajectoryBlendResponse &res,
                                                  ...
                                                  double joint_accleration_tolerance)
{
  ...
  {
    double blend_start_velo = (blend_start.positions.at(i) - first_end.positions.at(i))/blend_start.time_from_start.toSec();
    ...
    {
      std::cout << "Acceleration computed from positions/velocities are different from the value in trajectory." << std::endl
  ...
  {
    double second_start_velo = (second_start.positions.at(i) - blend_end.positions.at(i))/second_start.time_from_start.toSec();
...
bool testutils::checkBlendingCartSpaceContinuity(const pilz::TrajectoryBlendRequest &req,
                                                 const pilz::TrajectoryBlendResponse &res,
                                                 const pilz::LimitsContainer& planner_limits)
{
  ...
  pose_blend_end_1 = res.blend_trajectory->getWayPointPtr(res.blend_trajectory->getWayPointCount()-2)->getFrameTransform(req.link_name);
  ...
  if(v_2.norm() > max_trans_velo)
  {
    std::cout << "Translational velocity between first trajectory and blend trajectory exceeds the limit."
              << "Actual velocity (norm): " << v_2.norm() << "; " << "Limits: " << max_trans_velo << std::endl;
  }
  // rotational velocity
  if(w_2.norm() > max_rot_velo)
  {
    std::cout << "Rotational velocity between first trajectory and blend trajectory exceeds the limit."
              << "Actual velocity (norm): " << w_2.norm() << "; " << "Limits: " << max_rot_velo << std::endl;
  }
  ...
  if( a_1.norm() > max_trans_acc || a_2.norm() > max_trans_acc)
  {
    std::cout << "Translational acceleration between first trajectory and blend trajectory exceeds the limit."
              << "Actual acceleration (norm): " << a_1.norm() << ", " << a_1.norm() << "; " << "Limits: "
              << max_trans_acc << std::endl;
  }
                                        ...
                                        const pilz::TrajectoryBlendResponse& res)
{
  ...
  {
    Eigen::Affine3d curr_pos(res.blend_trajectory->getWayPointPtr(i)->getFrameTransform(link_name));
...
{
  initialJointState = testutils::generateJointState({0., 0.007881892504574495, -1.8157263253868452,
...
bool testutils::getBlendTestData(const ros::NodeHandle &nh,
...
bool testutils::generateTrajFromBlendTestData(const robot_model::RobotModelConstPtr& robot_model,
                                              ...
                                              planning_interface::MotionPlanResponse& res_1,
                                              ...
                                              double& dis_2)
{
  ...
  req_1.goal_constraints.push_back(
        kinematic_constraints::constructGoalConstraints(goal_state_1,
                                                        goal_state_1.getRobotModel()->getJointModelGroup(group_name)));
  ...
  req_2.goal_constraints.push_back(
        kinematic_constraints::constructGoalConstraints(goal_state_2,
                                                        goal_state_2.getRobotModel()->getJointModelGroup(group_name)));
  // trajectory generation
  ...
  dis_1 = (start_state.getFrameTransform(link_name).translation()
...
bool testutils::checkBlendResult(const pilz::TrajectoryBlendRequest& blend_req,
                                 ...
                                 double cartesian_angular_velocity_tolerance)
{
  ...
  if(!testutils::checkBlendingJointSpaceContinuity(blend_res,
  ...
  if(!testutils::checkBlendingCartSpaceContinuity(blend_req,
                                                  blend_res,
                                                  limits))
  {
    return false;
