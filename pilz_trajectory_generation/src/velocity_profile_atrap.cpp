...
VelocityProfile_ATrap::VelocityProfile_ATrap(double max_vel, double max_acc, double max_dec)
  : max_vel_(fabs(max_vel)), max_acc_(fabs(max_acc)), max_dec_(fabs(max_dec)),
    start_pos_(0), end_pos_(0), start_vel_(0),
    a1_(0), a2_(0), a3_(0),
    b1_(0), b2_(0), b3_(0),
    c1_(0), c2_(0), c3_(0),
    t_a_(0),t_b_(0),t_c_(0)
{
}
...
bool VelocityProfile_ATrap::SetProfileStartVelocity(double pos1, double pos2, double vel1)
{
  ...
  double min_dis_max_vel = 0.5*(max_vel_ - start_vel_)*(max_vel_ + start_vel_)/max_acc_ + 0.5*max_vel_*max_vel_/max_dec_;
  double dis = fabs(end_pos_ - start_pos_);
  ...
  else if (dis <= min_dis_max_vel)
  {
    ...
    double new_vel = s*sqrt( (dis+0.5*start_vel_*start_vel_/max_acc_)*2.0*max_acc_*max_dec_/(max_acc_+max_dec_) );
...
{
  return (max_vel_ == other.max_vel_ &&
          max_acc_ == other.max_acc_ &&
          max_dec_ == other.max_dec_ &&
          start_pos_ == other.start_pos_ &&
          end_pos_ == other.end_pos_ &&
          start_vel_ == other.start_vel_ &&
          a1_ == other.a1_ &&
          a2_ == other.a2_ &&
          a3_ == other.a3_ &&
          b1_ == other.b1_ &&
